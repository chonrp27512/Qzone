...
public class NineImgLayout extends ViewGroup {
    ...
    private void init(){
       ...
       // this.setBackgroundColor(getResources().getColor(R.color.baise));
    ...
    public void displayPics(){
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (i >= picRects.length) {
                ...
                ImageLoader.getInstance().displayImage(url, imgView, ImageConfigBuilder.USER_HEAD_HD_OPTIONS);
                //Glide.with(getContext()).load(url).error(R.mipmap.ic_launcher).into(imgView);
    ...
    public void checkPicSize() {
        if (picUrls != null && picUrls.size() == 1) {
            ...
            if (file.exists()) {
                ...
                oneOneWidth = opts.outWidth;
                oneOneHeight = opts.outHeight;
                // 重新规划尺寸
                calculatePicSize();
            }
        }
    ...
    //列表滑动时，不加载图片
    private void scrolling(boolean scoll){
        if(scoll){ //正在滑动
